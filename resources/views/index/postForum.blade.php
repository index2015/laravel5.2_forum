...
<div class="content forumdisplay">
    <div class="box">
        <div class="forum_top_icon"><img src="{{$barInfo->thumbnail}}" alt="{{$barInfo->bar_name}}" height="140" width="140"></div>
        <div class="forum_top_name">
            ...
            <h2><a href="forum-2-1.html">{{$barInfo->bar_name}}</a></h2>
            <p>{{$barInfo->describe}}</p>
        </div>
        <div class="forum_top_info">
            ...
            <div class="right">
                ...
                <span class="pipe" style="margin:0 8px;">|</span></div>今日: {{$barInfo->today}}<span class="pipe">|</span>主题:{{$barInfo->theme}}</div>
    </div>
    ...
    <div class="main">
        ...
        <div class="box main_box">
            ...
            <div class="oneway_text graybottomborder">
                ...
                <ul>
                    <li><a href="{{url('forum/postForum?id='.$_GET['id'].'&postAddTime=postAddTime')}}"><i class="newthead"></i>最新</a></li>

                    <li><a href="{{url('forum/postForum?id='.$_GET['id'].'&postTop=1')}}"><i class="hotthead"></i>热门</a></li>
                    <li><a href="{{url('forum/postForum?id='.$_GET['id'].'&postEssence=1')}}"><i class="verythead"></i>精华</a></li>
                </ul>
            </div>
            ...
            <div class="thread minh flow-default">
                <form method="post" autocomplete="off" name="moderate" id="moderate" action="#">
                    @foreach($dataList as $k=>$vo)
                        <div class="threadlist" id="stickthread_{{$k+1}}"  >
                            <div class="threadlist_stat"><a href="{{url('forum/showPost?id='.$vo['id'].'&postBar='.$vo['postBar'])}}" title="0 个回帖" >{{$vo['read']}}</a></div>
                            <div class="threadlist_title">
                                <div class="right"></div>
                                <a href="{{url('forum/showPost?id='.$vo['id'].'&postBar='.$vo['postBar'])}}"  target="" class="gr" >{{$vo['postName']}}</a>
                            ...
                            <div class="threadlist_imglist">
                                <div>
                                    <a href="{{url('forum/showPost?id='.$vo['id'].'&postBar='.$vo['postBar'])}}">
                                        <?php echo $vo['postContent'];?>
                                    </a>
                                </div>
                            </div>
                            <div class="threadlist_info">
                                <div class="lastreply"><a href="#"> {{$vo['userName']}} @ {{date('Y-m-d H:i:s',$vo['postAddTime'])}}</a></div>
        ...
        <div class="pg" >
            ...
            {!! $dataList->appends(['id' =>$_GET['id']])->links()  !!}
    ...
    <div class="sidebar">
        <div class="box p20">
            <a href="{{url('forum/newthread?id=')}}{{$barInfo->PId}}" class="bluebigbutton" title="发新帖">发新帖</a>
        </div>
